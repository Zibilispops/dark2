...
## Antigravity Design Engine & Stitch Integration

Antigravity uses the **Stitch Framework** to rapidly prototype and iterate on UI designs. This skill integrates Stitch tools to ensure design consistency and high-fidelity output.

### 1. Project Initialization
When starting a new design task, use `mcp_stitch_create_project` to establish a container for your work.

### 2. Design System Configuration
Use `mcp_stitch_create_design_system` to define the foundational tokens:
- **Colors**: Use OKLCH or premium curated palettes.
- **Typography**: Inter, Roboto, or Outfit for a modern look.
- **Shapes**: High corner roundness (12px-24px) for a premium feel.
- **Appearance**: Sleek dark mode by default.

### 3. Screen Generation & Iteration
- Use `mcp_stitch_generate_screen_from_text` for initial layouts.
- Refine using `mcp_stitch_edit_screens` with specific design prompts (e.g., "Apply glassmorphism to the sidebar", "Add a bento-grid layout to the dashboard").
- Generate variations with `mcp_stitch_generate_variants` to explore different aesthetics.

### 4. Branding & Visual Excellence
Leverage `generate_image` to create unique brand assets, hero illustrations (glass/3D style), and custom backgrounds to replace generic placeholders.
