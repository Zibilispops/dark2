...
function getStockLabel(slug: string, t: (k: TranslationKey) => string): { label: string; urgent: boolean } {
  const count = STOCK_COUNTS[slug] ?? 12;
  if (count <= 8) return { label: `${count} ${t('shop.remaining')}`, urgent: true };
  if (count <= 12) return { label: `${count} ${t('shop.remaining')}`, urgent: false };
  return { label: t('shop.limited'), urgent: false };
}
...
function ProductCard({ product, i, columns = 4, t }: { product: Product; i: number; columns?: number; t: (k: TranslationKey) => string }) {
  const isFeatured = FEATURED_SLUGS.has(product.slug);
  const stock = getStockLabel(product.slug, t);
    ...
    >
      ...
      <div className="product-card-border aspect-[3/4] overflow-hidden bg-[#0f0f0f] relative border border-white/5 rounded-sm mb-3">
        ...
        <div className="absolute top-2 left-2 right-2 flex justify-between items-start z-10">
          ...
          <span className="bg-black/60 border border-white/10 font-mono text-[8px] uppercase tracking-widest px-1.5 py-0.5 text-white/80">
            Bad Printer 6.6oz
          </span>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 px-3 py-2 flex justify-between items-end bg-gradient-to-t from-black/80 to-transparent">
          <span className="font-mono text-[8px] uppercase tracking-widest text-white/60 border border-white/5 px-1.5 py-0.5">
            {product.id}
          </span>
      ...
      <div className="px-1 mt-2">
        ...
        <p className="text-white/40 group-hover:text-white/80 text-[10px] uppercase font-mono tracking-widest transition-colors duration-700">
          {product.sizes.includes('ONE SIZE') ? t('shop.size.one') : t('shop.size.range')}
        </p>
      </div>
      </Link>
    </motion.div>
...
export function ShopGrid({ products }: { products: Product[] }) {
  ...
  // Localised filter labels — key stays 'ALL' / 'RAMEN' etc for filtering logic
  const FILTER_LABELS: Record<string, string> = {
    'ALL':        t('shop.filter.all'),
    'RAMEN':      t('shop.filter.ramen'),
    'ICE CREAM':  t('shop.filter.ice'),
    'CYBERPUNK':  t('shop.filter.cyber'),
    'ABSTRACT':   t('shop.filter.abstract'),
    'FOOD & DRINK': t('shop.filter.food'),
  };
  ...
  return (
    <>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
            {featured.map((product, i) => (
              <ProductCard key={product.id} product={product} i={i} columns={3} t={t} />
            ))}
          </div>
      ...
      <motion.p
        ...
        viewport={{ once: true }}
        className="text-white/20 font-mono text-[10px] uppercase tracking-widest mb-6"
      >
        {lang === 'ja'
          ? `${filtered.length}${t('shop.count.items')}${active !== 'ALL' ? ` · ${t('shop.count.in')}${FILTER_LABELS[active]}` : ` · ${t('shop.count.available')}`}`
          : `${filtered.length} ${filtered.length !== 1 ? t('shop.count.items') : t('shop.count.item')} ${active !== 'ALL' ? `in ${active}` : t('shop.count.available')}`
        }
      </motion.p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-x-6 gap-y-12">
        {filtered.map((product, i) => (
          <ProductCard key={product.id} product={product} i={i} columns={4} t={t} />
        ))}
      </div>
    </>
  );
