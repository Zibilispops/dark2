    ...
    <main className="min-h-screen bg-[#080808] text-white pt-24 pb-24 px-6 md:px-16">
      <div className="max-w-3xl">
        ...
        <div className="space-y-16">
          ...
          <section>
            <FadeUp delay={0}>
              <p className="font-mono text-[9px] uppercase tracking-widest text-[var(--accent)] mb-8">
              ...
              {shippingSectors.map((sector, sIdx) => (
                <FadeUp key={sector.title} delay={0.1 + (sIdx * 0.1)}>
                  <div className="border border-white/5 p-6 bg-[#0c0c0c]">
                    <h3 className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#666] mb-4 border-b border-white/5 pb-2">
                      {sector.title}
                    </h3>
                      ...
                      {sector.rows.map((row, rIdx) => (
                        <div key={rIdx} className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-2 border-b border-white/5 last:border-0 group">
                          <div className="flex flex-col">
                            <span className="font-mono text-[10px] uppercase tracking-widest text-white group-hover:text-[var(--accent)] transition-colors">
                              {row.service}
                            </span>
                            <span className="font-mono text-[9px] uppercase tracking-widest text-[#444] mt-1">
                              {row.time}
                            </span>
                          </div>
                          <span className="font-mono text-[11px] uppercase tracking-widest text-white mt-1 sm:mt-0 italic font-black">
                            {row.rate}
                          </span>
                        </div>
                      ))}
            ...
            <FadeUp delay={0.4}>
              <p className="mt-4 font-mono text-[8px] uppercase tracking-[0.2em] text-[#333] leading-relaxed">
            ...
            <FadeUp delay={0.1}>
              <div className="border border-white/5 p-6 bg-[#0c0c0c] space-y-6">
                {trackingInfo.map((info, i) => (
                  <div key={i}>
                    <p className="font-mono text-[10px] uppercase tracking-widest text-white mb-2">{info.type}</p>
                    <p className="font-mono text-[9px] uppercase tracking-widest text-[#555] leading-relaxed">{info.details}</p>
                  </div>
                ))}
                <p className="font-mono text-[8px] uppercase tracking-[0.2em] text-[#333] pt-2 border-t border-white/5">
            ...
            <FadeUp delay={0.1}>
              <div className="border border-white/5 p-6 bg-[#0c0c0c] space-y-6">
                {deliveryMethods.map((method, i) => (
                  <div key={i}>
                    <p className="font-mono text-[10px] uppercase tracking-widest text-white mb-2">{method.method}</p>
                    <p className="font-mono text-[9px] uppercase tracking-widest text-[#555] leading-relaxed">{method.details}</p>
                  </div>
                ))}
              </div>
            </FadeUp>
            ...
            <FadeUp delay={0.1}>
              <div className="border-l-2 border-[var(--accent)] bg-[#0c0c0c] p-6 space-y-8">
                {importantNotices.map((notice, i) => (
                  <div key={i}>
                    <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-[var(--accent)] mb-2">• {notice.label}</p>
                    <p className="font-mono text-[9px] uppercase tracking-widest text-[#666] leading-relaxed italic">{notice.text}</p>
                  </div>
                ))}
              </div>
            </FadeUp>
